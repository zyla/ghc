...
unflatten tv_eqs funeqs
 = do { tclvl    <- getTcLevel
  ...
  where
    ...
    unflatten_eq tclvl ct@(CTyEqCan { cc_ev = ev, cc_tyvar = tv
      ...
      = do { is_filled <- isFilledMetaTyVar tv
           ; elim <- case (is_filled, tcGetTyVar_maybe rhs) of
               (False, _)          -> tryFill      eq_rel       tv rhs    ev
               (True, Just rhs_tv) -> try_fill_rhs eq_rel tclvl tv rhs_tv ev
    ...
    ----------------
    try_fill_rhs eq_rel tclvl lhs_tv rhs_tv ev
      ...
      = do { is_filled <- isFilledMetaTyVar rhs_tv
           ; if is_filled then return False
             else tryFill eq_rel rhs_tv (mkTyVarTy lhs_tv) ev }
...
tryFill :: EqRel -> TcTyVar -> TcType -> CtEvidence -> TcS Bool
