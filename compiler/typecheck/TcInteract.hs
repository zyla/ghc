...
reduce_top_fun_eq old_ev fsk (ax_co, rhs_ty)
  ...
  | otherwise   -- So old_ev is Wanted or Derived
...
doTopReactDict inerts work_item@(CDictCan { cc_ev = fl, cc_class = cls
  ...
  | not (isWanted fl)   -- Never use instances for Given or Derived constraints
                        -- See Note [No reduction for Derived class constraints]
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
If we have a Derived class constraint (via a superclass of a Wanted),
should we reduce it?  It's just possible that doing so might be
useful.  E.g. typecheck/should_compile/Improvement.hs
  instance (b~Int) => C Bool b
  type instance F Int = Bool

with constraints (a0 is a unification variable):
  [W] C (F a0) a0
  [W] F a0 ~ Bool

If we make a derived constraint [D] C (F a0) a0, and substitute
for (F a0) (via flattening etc), we'd get [D] C Bool a0. Now
reduce via the instance to get a0~Int; and that solves everything.

It's not hard to implement this, but it makes extra work and is quite
hard to predict, so I doubt programmers are going to rely on it.  We'll
see.

Also there /are/ no Derived CFunEqCans, so it's symmetrical with that.
See Note [No Derived CFunEqCans] in TcSMonad.
-}
